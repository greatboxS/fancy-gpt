...
from .project_models import AgentAssignment, AgentOutcome, AgentOutcomeStatus, AgentRole
...
"""
    ...
    def run(self, assignment: AgentAssignment, provider: AutomaticModelProvider, *, request_id: str | None = None) -> AgentOutcome:
        ...
        try:
            ...
            if response.conversation_binding:
                outcome = outcome.model_copy(update={"conversation_binding": response.conversation_binding})
            return outcome
        finally:
            if started:
                provider.stop()
