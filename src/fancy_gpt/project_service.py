...
class ProjectService:
    ...
    def start_session(
        ...
        *,
        ...
        if conversation_binding is None and strategy == ConversationStrategy.RESUME:
            ...
            real = [session for session in candidates if str(session.conversation_binding).startswith("https://chatgpt.com/")]
            if real:
    ...
    def bind_session_conversation(self, project_id: str, session_id: str, binding: str) -> SessionRecord:
        if not binding.startswith("https://chatgpt.com/") and not binding.startswith("fake://"):
            raise ValueError("conversation binding must be a ChatGPT URL or test binding")
