...
description: Support a technical decision with options, trade-offs, risks, and a recommendation.
compatibility: fancy-gpt MCP/CLI
---

# technical-consult

Use this skill when the requested reasoning mode is **consult**. Technical specialization belongs in the request `domains` and `focus`; do not create a new skill for a narrow technology.
...
Required report sections: problem-framing, options, trade-offs, recommendation, unknowns, validation-plan.

Quality gates:
- At least one concrete option analysis is required.
- State when each option should be chosen.
