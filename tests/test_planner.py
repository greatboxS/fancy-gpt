...
def test_design_candidate_is_absent_even_as_metadata(tmp_path: Path):
    req=RawRequest(mode="design",objective="design system",repo_root=str(tmp_path),domains=["architecture"],artifacts=[ArtifactSpec(path="candidate.md",role=ArtifactRole.CANDIDATE_SOLUTION)])
...
def test_final_prompt_does_not_reintroduce_candidate_or_host_path(tmp_path: Path):
    ...
    req=RawRequest(mode="design",objective="design cleanly",repo_root=str(tmp_path),domains=["architecture"],artifacts=[ArtifactSpec(name="candidate-secret-name",content="CANDIDATE BODY",role=ArtifactRole.CANDIDATE_SOLUTION),ArtifactSpec(name="requirements",content="REQ-1",role=ArtifactRole.REQUIREMENT)])
