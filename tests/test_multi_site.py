...
def test_glm_thinking_is_activity_not_a_response(tmp_path: Path) -> None:
    ...
    assert "Thought Process" in source
    assert "config.cleanResponse" in engine
...
def test_extra_sites_have_current_composer_and_submit_fallbacks(tmp_path: Path) -> None:
    ...
    assert 'a[aria-label="Switch to Private Chat"]' in grok
    assert "This chat won.t appear in your history" in grok
    ...
    assert "baselineNodes = new Map" in engine
    ...
    assert 'fresh: "https://grok.com/"' in background
