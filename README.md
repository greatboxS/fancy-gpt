...
Per-site readiness, integration ownership, and the contract used to join browser
work to the gateway are tracked in
...
- **Model Gateway** lets Codex, Claude Code, and Gemini CLI use a FancyGPT web model as their direct model provider.

The current delivery order and acceptance gates are tracked in [ROADMAP.md](docs/ROADMAP.md). Gateway setup, protocols, and context behavior are specified in [MODEL_GATEWAY.md](docs/MODEL_GATEWAY.md).
...
This installs directly from either the bundled wheel or the source checkout,
registers the MCP server with detected Codex/Claude Code/Gemini CLI clients, and configures
the local model gateway globally for Codex, Claude Code, and Gemini CLI. On Linux
with a user systemd session, the bridge and gateway are installed and started as
login services. Select a site with Codex's model picker after
`codex --profile fancy-gemini`, the `fancy-claude` launcher's `/model` picker, or
`gemini --model fancy-claude`. Use `--no-mcp-register`, `--no-gateway-config`,
or `--no-services` to opt out of an integration.

The default install includes Playwright Chromium as a ready-to-use local fallback.
For an extension-only or minimal CI installation, opt out explicitly:

```bash
./install.sh --without-playwright --no-services
```
...
A browser stops painting a document it is not showing. A reply read from the
rendered page therefore freezes part-written whenever the automation window is
not in front — measured on live turns at 21 to 61 characters of a 1294
character answer, with the site's own stop control frozen alongside it, so the
turn looked finished while holding a fragment.

So a turn is read two ways at once.

- **From the page.** The adapter reads the rendered reply, reinstating the line
  structure a layout pass would have produced and keeping a fenced block's
  indentation byte for byte, because the code-change contract matches it
  literally. This is the source of truth today.
- **From the stream.** A content script running in the page's own world
  captures the events the site streamed and forwards them unread; the runtime
  decodes them. The network does not stop in a hidden tab, only the painting
  does.

Both run on every turn and the two readings are compared, so the page path
stays a fallback that is known to work rather than one discovered to be
unreachable later. A decoder that meets an operation it does not recognise, or
a fragment it cannot place, marks its result untrusted and the page answers
instead: refusing costs a fallback, guessing costs a wrong answer that nothing
catches.

Decoding lives in `src/fancy_gpt/stream_decoding.py`, never in the extension.
It is the part that changes when a site changes, and a decoder inside an
extension can only be corrected by asking every user to reload their browser.

Adding a site follows [docs/ADDING_A_SITE.md](docs/ADDING_A_SITE.md), which is
a loop of measure-then-write for the same reason: a reply format written from
assumption yields text that reads correctly and is wrong.
...
```bash
fancy-gpt extension reload edge-remote
```

The command refuses to interrupt active turns and succeeds only after a new
worker reconnects with the expected build id. The first upgrade from an older
extension that predates this command still requires one manual reload.
